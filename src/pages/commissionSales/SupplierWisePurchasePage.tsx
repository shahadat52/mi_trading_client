...
const SupplierWisePurchasePage = () => {
    ...
    const handleExportExcel = () => {
        if (!commissionProducts) return;

        const rows = commissionProducts?.map((item: any) => ({
    ...
    return (
        <div className="p-5 bg-white shadow rounded-xl border">
            <h2 className="text-lg font-semibold mb-4">
                {commissionProducts?.[0]?.supplier?.name} এর সাপ্লাইঃ
            </h2>
            ...
            <div id="printArea" className="hidden">
                <h2 className="text-xl font-bold mb-4">
                    {commissionProducts?.[0]?.supplier?.name} এর কমিশন ক্রয় রিপোর্ট
                </h2>
                    ...
                    <tbody>
                        {commissionProducts?.map((item: any) => (
                            <tr key={item._id}>
                                ...
                                <td className="border px-2 py-1">{item?.commissionRate}</td>
