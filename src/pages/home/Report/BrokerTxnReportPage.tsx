...
import { useGetBrokerByIdQuery, useGetSpecificBrokerTxnQuery } from "../../../redux/features/broker/brokerApi";

const BrokerTxnReportPage = () => {
    const { id } = useParams();
    ...
    const { data, isLoading } = useGetSpecificBrokerTxnQuery({ id, startDate, endDate });
    const transactions = data?.data?.transactions || [];
    const { data: brokerData } = useGetBrokerByIdQuery(id);
    const customer = brokerData?.data || {};
    ...
    const totalDebit = transactions?.filter((item: any) => item.type === "debit")
        .reduce((sum: number, item: any) => sum + item.amount, 0);

    const totalCredit = transactions?.filter((item: any) => item.type === "credit")
        .reduce((sum: number, item: any) => sum + item.amount, 0);
        ...
        <div className="min-h-screen p-4 mx-auto">
            ...
            >
                ...
                <div className="text-center text-sm ">
                    <h2>Broker Transactions Statement</h2>
                ...
                <div className="grid grid-cols-2 mt-3 gap-2 text-sm">
                    <div>
                        ...
                        <b>Balance :</b> {totalCredit + totalDebit} <br />
                    </div>
...
export default BrokerTxnReportPage;