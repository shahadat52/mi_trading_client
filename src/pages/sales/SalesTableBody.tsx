    ...
    return (
        <React.Fragment key={row._id}>
            <tr className="border-t hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-2 text-right">{row.grandTotal}</td>
                <td className="px-4 py-2 text-right">{row.paidAmount || 0}</td>
                ...
                <td
                    ...
                    className={`cursor-pointer px-4 py-2 text-center p-2   rounded ${row?.isDelivered ? 'text-green-700' : 'text-red-700'}`}>{row?.isDelivered ? 'Yes' : 'No'}</td>
                <td className="px-4 py-2">
                    <div className="flex gap-2">
                        ...
                        <button onClick={() => { window.open(`/sales/print/${row._id}`, '_blank'); }} className="text-sm px-2 py-1 border rounded">View</button>
